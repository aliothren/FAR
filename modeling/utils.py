...
def save_train_fig(save_path, epoch, acc, curves, valid, step, mode):
    ...
    ax3.legend(
        ("Elt_sparsity", "Filter_sparsity", "Average_sparsity"), loc="lower right"
    )
...
def print_nonzeros(model):
    ...
    for name, p in model.named_parameters():
        if "weight" in name:
            ...
            if tensor.ndim < 2:
                print(
                    f"{name:20} | Skipped channel-wise sparsity (not a matrix): shape = {tensor.shape}"
                )
                continue
            ...
            print(f"{name:20} |")
            print(
                f"Element: nonzeros = {nz_count:7} / {total_params:7} ({100 * nz_count / total_params:6.2f}%) | total_pruned = {total_params - nz_count :7} | shape = {tensor.shape}"
            )

            # Channel-wise sparsity
            abs_tensor = np.abs(tensor)
            if abs_tensor.ndim == 4:
                reshaped = abs_tensor.reshape(
                    abs_tensor.shape[0], -1
            ...
            print(
                f"Channel: nz_rows = {num_nz_rows:7} / {num_rows:7} ({100 * num_nz_rows / num_rows:6.2f}%)"
            )
            print(
                f"Channel: nz_cols = {num_nz_cols:7} / {num_cols:7} ({100 * num_nz_cols / num_cols:6.2f}%)"
            )
            print(
                f"Channel: total_pruned = {total_params - effective_params :7} ({100 * effective_params / total_params}% left) | shape = {tensor.shape}"
            )
