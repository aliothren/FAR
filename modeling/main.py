...
def train(args, seq=0):
    ...
    if args.ds_in_train:
        ...
        if (
            ...
            and args.block_ft_train_loss == "classification"
        ):
            teacher_model = None
        else:
    ...
    try:
        ...
        print(f"Unable to print model structure: {e}")

    # Train attention part
    if not args.skip_train_attn:
        # Set trainable parameters
        print("Set teacher_model to trainable, blocks [], part attn")
        architectures.set_requires_grad(
            teacher_model, "train", [], "attn"
        )  # No trainable param in teacher
        print(f"Set student_model to trainable, blocks {args.replace}, part attn")
        architectures.set_requires_grad(
            student_model, "train", args.replace, "attn"
        )  # Target attn part trainable
        n_parameters = sum(
        ...
        if not args.unscale_lr:
            linear_scaled_lr = (
                args.lr * args.batch_size * utils.get_world_size() / 512.0
            )
    ...
    if args.block_ft:
        ...
        if args.reg_in_train:
            trained_model, pruning_mask = prune(
                args, trained_model_without_ddp, data_loader_val
            )
        else:
            pruning_mask = {}
        print("Doing block-level finetuning of attention-trained model...")

        # Continue training on trained_model
        print(
            f"Set trained_model to trainable, blocks {args.replace}, part {args.block_ft_mode}"
        )
        architectures.set_requires_grad(
            trained_model, "train", args.replace, args.block_ft_mode
        )  # Target part trainable
        print(f"Set teacher_model to trainable, blocks [], part {args.block_ft_mode}")
        architectures.set_requires_grad(
            teacher_model, "train", [], "block"
        )  # Not trainable
...
def downstream(args, pretrained_path):
    ...
    args.train_loss = "classification"
    print(
        f"Loading pretrained model: {pretrained_path} with class num {args.nb_classes}"
    )
...
if __name__ == "__main__":
    ...
    if args.mode == "eval":
        ...
        if args.train_mode == "sequential":
            seq = 0
            print(
                f"Sequentially training blocks {args.replace}, {args.step} blocks once..."
            )
